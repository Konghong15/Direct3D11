...
ModelLoader::ModelLoader(HWND hwnd, ID3D11Device* d3dDevice, ID3D11DeviceContext* d3dImmdiateContext)
	: mhWnd(hwnd)
	, md3dDevice(d3dDevice)
	, md3dImmediateContext(d3dImmdiateContext)
{
}
...
bool ModelLoader::Load(std::string key, std::string fileName)
{
...
void ModelLoader::processNodeRecursive(aiNode* node, const aiScene& scene, Node* parent) const
{
	assert(node != nullptr);
...
Mesh ModelLoader::processMesh(aiMesh* mesh, const aiScene& scene) const
{
...
void ModelLoader::processAnimation(const aiScene* scene, Model* model) const
{
...
void ModelLoader::processBones(Model* model) const
{
...
void ModelLoader::registerNodeRecursive(const Node* current, std::map<std::string, const Node*>* nodeMap) const
{
...
void ModelLoader::connectBoneNodeRecuresive(Node* current, const std::map<std::string, const Node*>& nodeMap) const
{
	assert(current != nullptr);
...
std::vector<Texture> ModelLoader::loadMaterialTextures(aiMaterial* mat, aiTextureType type, std::string typeName, const aiScene& scene) const
{
...
ID3D11ShaderResourceView* ModelLoader::loadEmbeddedTexture(const aiTexture* embeddedTexture) const
{
	HRESULT hr;
...
DirectX::SimpleMath::Matrix ModelLoader::convertMatrix(const aiMatrix4x4& aiMatrix) const
{
