...
namespace common
{
	...
	Terrain::~Terrain()
	{
		ReleaseCOM(mFrameTerrainCB);
		ReleaseCOM(mObjectTerrainCB);
		ReleaseCOM(mTerrainVS);
		ReleaseCOM(mTerrainHS);
		ReleaseCOM(mTerrainDS);
		ReleaseCOM(mTerrainPS);
		ReleaseCOM(mTerrainIL);
		ReleaseCOM(mSamLinear);
		ReleaseCOM(mSamHeightMap);
	...
	void Terrain::buildTerrain(ID3D11Device* device)
	{
		...
		// 쉐이더 컴파일
		ID3D10Blob* blob;
		HR(D3DHelper::CompileShaderFromFile(L"../Resource/Shader/Terrain.hlsl", "VS", "vs_5_0", &blob));
		...
		HR(D3DHelper::CompileShaderFromFile(L"../Resource/Shader/Terrain.hlsl", "HS", "hs_5_0", &blob));
		...
		HR(D3DHelper::CompileShaderFromFile(L"../Resource/Shader/Terrain.hlsl", "DS", "ds_5_0", &blob));
		...
		HR(D3DHelper::CompileShaderFromFile(L"../Resource/Shader/Terrain.hlsl", "PS", "ps_5_0", &blob));
